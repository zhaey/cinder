...
def genLevelInfoJSON(header, level):
    info = '{'
    info += '"_difficulty":"' + level[0]['diff'] + '",'
    info += '"_difficultyRank":' + level[0]['diffNum'] + ','  # TODO
    # info += '"audioPath":"' + header['MUSIC'] + '",'
    info += '"_beatmapFilename":"Standard' + level[0]['diff'] + '.dat",'
    info += '"_noteJumpMovementSpeed":' + NOTE_JUMP_SPEED + ','
    info += '"_noteJumpStartBeatOffset":' + NOTE_JUMP_OFFSET
    info += '},'
...
def genInfoJSON(header, levels):
    ...
    if 'TITLETRANSLIT' in header.keys() and header['TITLETRANSLIT'] != '':
        info += '"_songName":"' + header['TITLETRANSLIT'] + '",'
    else:
        info += '"_songName":"' + header['TITLE'] + '",'

    if 'SUBTITLETRANSLIT' in header.keys() and header['SUBTITLETRANSLIT'] != '':
        info += '"_songSubName":"' + header['SUBTITLETRANSLIT'] + '",'
    elif 'SUBTITLE' in header.keys():
        info += '"_songSubName":"' + header['SUBTITLE'] + '",'
    else:
        info += '"_songSubName":"",'

    if 'ARTISTTRANSLIT' in header.keys() and header['ARTISTTRANSLIT'] != '':
        info += '"_songAuthorName":"' + header['ARTISTTRANSLIT'] + '",'
    else:
        info += '"_songAuthorName":"' + header['ARTIST'] + '",'

    info += '"_levelAuthorName":"",'
    info += '"_songTimeOffset":0,'
    info += '"_shuffle":' + SHUFFLE + ','
    info += '"_shufflePeriod":' + SHUFFLE_PERIOD + ','
    info += '"_beatsPerMinute":' + str(SONG_SPEED) + ','
    info += '"_previewStartTime":' + header['SAMPLESTART'] + ','
    info += '"_previewDuration":' + header['SAMPLELENGTH'] + ','
    info += '"_songFilename":"' + header['MUSIC'] + '",'
    info += '"_coverImageFilename":"' + COVER + '",'
    info += '"_environmentName":"DefaultEnvironment",'
    info += '"_difficultyBeatmapSets": [{'
    info += '"_beatmapCharacteristicName": "Standard",'
    info += '"_difficultyBeatmaps": ['
    for level in levels:
            ...
            for stop in sStops:
                if not stopped[counter]:
                    if stop[0] < curSongBeat:
                        ...
                        print(str(stop[0]) + " STOPPED at beat " + str(curSongBeat) + " curSeconds " + str(
                            secondsPassed) + " for " + str(stop[1]))
...
def exportLevelJSON(levels, dir):
    for level in levels:
        level = level.split('|')
        print("Writing difficulty file Standard" + level[0] + ".dat")
        infoStore = dir + '/Standard' + level[0]

        print(infoStore)

        with open(infoStore + '.dat', 'w') as levelFile:
...
def main():
    ...
    parser.add_argument('--dir', action='store',
                        help='specify a dir containing an sm file and possible image. New files are created in-place')
